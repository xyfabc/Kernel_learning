...
static unsigned char spi_flash_addr_size(void)
{
	...
	serial_puts("get addr size cmd save to dr\n");
	while (__ssi_get_rxfifo_count(IDX) != 5);
	serial_puts("get addr size cmd to rxfifo\n");
#else
	int count = 4;

	REG_SSI_DR(IDX) = CMD_READ;
	while (__ssi_txfifo_full(IDX));
	while(count--) {
		REG_SSI_DR(IDX) = 0;
		while (__ssi_txfifo_full(IDX));
	}
	...
	__ssi_flush_fifo(IDX);
	serial_puts("flush send fifo ok\n");

	while (!__ssi_rxfifo_empty(IDX));
	serial_puts("send fifo is empty\n");

	jz_spi_write_dummy_and_read(&addr_size, 1);
	serial_puts("read addr size ok:\n");
	//serial_put_hex(addr_size);
...
{
	//if (REG_CPM_CPCCR & CPM_CPCCR_PCS)		//twxie
...
static void __gpio_as_spi(void)
{
	REG_GPIO_PXINTC(0) =  0x9C0000;
	REG_GPIO_PXMASKC(0) = 0x9C0000;
	REG_GPIO_PXPAT1S(0) = 0x9C0000;
	REG_GPIO_PXPAT0C(0) = 0x9C0000;
}
