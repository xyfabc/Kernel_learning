...
int nand_write_opts(nand_info_t *meminfo, const nand_write_options_t *opts)
{
	...
	u_char *data_buf = NULL, *oob_buf = NULL;
	int buf_alloc = 0;
	size_t written;
	int result;
	...
	if (!data_buf) {
		data_buf = kmalloc(meminfo->oobblock + meminfo->oobsize, GFP_KERNEL);
		if (!data_buf) {
			printf("%s Cannot allocate data_buf.\n", __FUNCTION__);
			return -1;
		}
	...
	if (!oob_buf)
		oob_buf = data_buf + meminfo->oobblock;
...
#if defined(CONFIG_JZ4760B) || defined(CONFIG_JZ4770) || defined(CONFIG_JZ4780)
...
int nand_read_opts(nand_info_t *meminfo, const nand_read_options_t *opts)
{
	...
	if (!(meminfo->oobsize == 16 && meminfo->oobblock == 512)
	    ...
	    && !(meminfo->oobsize == 218 && meminfo->oobblock == 4096)
	    && !(meminfo->oobsize == 436 && meminfo->oobblock == 8192)
	    && !(meminfo->oobsize == 640 && meminfo->oobblock == 8192)
	    && !(meminfo->oobsize == 1024 && meminfo->oobblock == 8192)
	    && !(meminfo->oobsize == 1280 && meminfo->oobblock == 16384)) {
		printf("Unknown flash (not normal NAND)\n");
		return -1;
	}
