...
static struct nand_oobinfo nand_oob_640 = {
	.useecc = MTD_NANDECC_AUTOPLACE,
	.eccbytes = 1,
	.eccpos = {0},
	.oobfree = {{2, 1}, {560, 80}}
...
static struct nand_oobinfo nand_oob_1024 = {
	.useecc = MTD_NANDECC_AUTOPLACE,
	.eccbytes = 1,
	.eccpos = {0},
	.oobfree = {{2, 1}, {840, 184}}
...
static struct nand_oobinfo nand_oob_1280 = {
	.useecc = MTD_NANDECC_AUTOPLACE,
	.eccbytes = 1,
	.eccpos = {0},
	.oobfree = {{2, 1}, {1120, 160}}
};
...
#ifdef CFG_NAND_TOGGLE
		this->hwcontrol(mtd, TNAND_READ_PERFORM);
#endif
		if (this->read_byte(mtd) != 0xff)
			res = 1;
#ifdef CFG_NAND_TOGGLE
		tmp_read = this->read_byte(mtd);
		this->hwcontrol(mtd, NAND_CTL_CLRNCE);
#endif
	}
	...
	u_char *oob_buf,  struct nand_oobinfo *oobsel, int cached)
{
	...
	int	datidx, eccidx, eccsteps = this->eccsteps;
	u_char	ecc_code[MAX_ECC_BYTES];
	u_char *free_p = NULL;
	int	eccbytes = 0;
	int	count = 0;
	int 	i, status;
...
#ifdef CFG_NAND_TOGGLE
	this->hwcontrol(mtd, TNAND_WRITE_PERFORM);
#endif
	...
	/* Hardware BCH ECC, write all */
	case NAND_ECC_HW_BCH64:
		eccbytes = this->eccbytes;
		free_p = this->data_buf + mtd->data_per_page; 
		memset(free_p, 0x00, mtd->freesize);

		for (count = 0; count < eccsteps; count++) {
			/* enable hardware ecc logic for write */
			this->enable_hwecc(mtd, NAND_ECC_WRITE);
			this->calculate_ecc(mtd, &this->data_poi[datidx], ecc_code);
			for (i = 0; i < eccbytes; i++, eccidx++) {
#ifdef CFG_NAND_BCH_WITH_OOB
				if ((CFG_NAND_ECC_POS + (count + 1) * eccbytes + i) < mtd->oobsize)
					oob_buf[CFG_NAND_ECC_POS + (count + 1) * eccbytes + i] = ecc_code[i]; 
				else
					*free_p++ = ecc_code[i];
#else
				if ((CFG_NAND_ECC_POS + count * eccbytes + i) < mtd->oobsize)
					oob_buf[CFG_NAND_ECC_POS + count * eccbytes + i] = ecc_code[i]; 
				else
					*free_p++ = ecc_code[i];
#endif
			}
...
#if 0 /* CFG_NAND_BCH_WITH_OOB */
		this->enable_hwecc(mtd, NAND_ECC_WRITE_OOB);
		this->calculate_ecc(mtd, oob_buf, ecc_code);
		for (i = 0; i < eccbytes; i++)
			oob_buf[CFG_NAND_ECC_POS + i] = ecc_code[i];
#endif

		if (mtd->freesize)
			this->write_buf(mtd, free_p, mtd->freesize);
		break;
			     ...
			     struct nand_oobinfo *oobsel)
{
	...
	u_char *free_pos = NULL, *ecc_poi = NULL;
...
//		mem_dump(data_poi, 512);
		...
		case NAND_ECC_HW_BCH64 :
			...
			ecc_poi = this->data_buf + end + CFG_NAND_ECC_POS;
			memset(this->data_buf + end + mtd->oobsize, 0xff, mtd->freesize);
//			if (data_poi != this->data_buf)
//				memcpy(ecc_poi, &data_poi[end + CFG_NAND_ECC_POS],
//					mtd->oobsize - CFG_NAND_ECC_POS); //xlsu
			if (mtd->freesize)
				memcpy(&ecc_poi[mtd->oobsize - CFG_NAND_ECC_POS],
					free_pos, mtd->freesize);

			for (i = 0, datidx = 0; i < this->eccsteps; i++, datidx += ecc) {
				this->enable_hwecc(mtd,NAND_ECC_READ);
				ecc_status = this->correct_data(mtd, &data_poi[datidx], &ecc_poi[j], NULL);
				j += eccbytes;
				if ((ecc_status == -1) || (ecc_status > 0xff)) {	
...
int nand_read_raw (struct mtd_info *mtd, uint8_t *buf, loff_t from, size_t len, size_t ooblen)
{
	...
	while (len) {
		...
		this->read_buf (mtd, &buf[count], pagesize);
#ifdef CFG_NAND_TOGGLE
			this->hwcontrol(mtd, NAND_CTL_CLRNCE);
#endif
...
#ifdef CFG_NAND_TOGGLE
	chip->hwcontrol(mtd, TNAND_READ_PERFORM);
#endif
...
//	chip->cellinfo = ext_id[0];
	...
	printk ( "  Device ID: [dev id: 0x%04x] [ext_id 0x%06x]\n       ", dev_id, *ext_id_p);
#define EXTID_MASK	0x00ffffff
#define NAND_LOW_CHIP_SIZE	2048
	...
	/* Check NAND chip size,if it is larger than 2048 MiB and off 32 bit
	 * use chip size 2048 MiB
	 */
	chip->chipsize = (uint64_t)type->erasesize * type->maxvalidblocks;
	if (chip->chipsize == 0 && type->erasesize != 0 && type->maxvalidblocks != 0){
		chip->chipsize = NAND_LOW_CHIP_SIZE << 20;
		printk("NAND chip size > NAND_LOW_CHIP_SIZE, set NAND chip size = NAND_LOW_CHIP_SIZE: ");
	}
	...
	chip->ppb =  type->erasesize / type->pagesize;
...
int nand_scan (struct mtd_info *mtd, int maxchips)
{
	...
	if (!this->autooob) {
		...
		case 436:
			...
			break;
		case 640:
			this->autooob = &nand_oob_640;
			break;
		case 1024:
			this->autooob = &nand_oob_1024;
			break;
		case 1280:
			this->autooob = &nand_oob_1280;
			break;
		default:
	...
	case NAND_ECC_HW_BCH64:
		if (mtd->oobblock == 256) {
			printk (KERN_WARNING "1024 byte HW ECC not possible on 256 Byte pagesize, fallback to SW ECC \n");
			this->eccmode = NAND_ECC_SOFT;
			this->calculate_ecc = nand_calculate_ecc;
			this->correct_data = nand_correct_data;
		} else if (mtd->oobblock == 512)
	 ...
	 * Check hardware ecc function availability and adjust number of ecc bytes per
        ...
        switch (this->eccmode) {
        case NAND_ECC_HW_BCH64:
                this->eccbytes = 14 * CFG_NAND_BCH_BIT / 8; 
                break;
	...
	switch (this->eccmode) {
	case NAND_ECC_HW_BCH64:
		this->eccsteps = mtd->data_per_page / mtd->eccsize;
		break;
	case NAND_ECC_HW3_256:
	case NAND_ECC_SOFT:
	...
	if (!this->data_buf) { /* this->data_buf size need pagesize + oobsize + freesize */
		size_t len;
		len = mtd->oobblock + mtd->oobsize + mtd->freesize;
