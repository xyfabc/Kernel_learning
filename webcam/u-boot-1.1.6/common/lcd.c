...
//#  error Default Color Map overlaps with Logo Color Map
	...
	#if 0
		#include <logo_test_yjt.h>
	#else
		#include <logo.newC330.h>
	#endif
...
#define LCD_HIGH	240
	for(x=sx, i=0; x < ex; x++, i++)	// 0 ~ 240-1
	{
		for(y=sy, j=0; y < ey; y++, j++)
		{
			ptr[x * LCD_WIDTH * 4 + (LCD_WIDTH - y) * 4 + 0] = data[i * pic_high * 4 + j * 4 + 0];
			ptr[x * LCD_WIDTH * 4 + (LCD_WIDTH - y) * 4 + 1] = data[i * pic_high * 4 + j * 4 + 1];
			ptr[x * LCD_WIDTH * 4 + (LCD_WIDTH - y) * 4 + 2] = data[i * pic_high * 4 + j * 4 + 2];
		}
	}
#else
...
/*
static int show_hw_logo(int sx, int sy, int ex, int ey, unsigned char *data)
{
	unsigned char *ptr;
	ptr = lcd_base; //(unsigned char *)lcd_frame[0];
	int y,x,i,j;
	int datap = 0;
	...
	for(j=0; j<320; j++)
	{
		for(i=0; i<240; i++)
		{
			ptr[240*j*4 + i*4+0]=data[240*j*4+i*4+0];
			ptr[240*j*4 + i*4+1]=data[240*j*4+i*4+1];
			ptr[240*j*4 + i*4+2]=data[240*j*4+i*4+2];
			ptr[240*j*4 + i*4+3]=data[240*j*4+i*4+3];
		}
	}
...
#if 0	// defined(CONFIG_JZ_LCD_BL35026V0)
...
ulong lcd_setmem (ulong addr)
{
	...
	size = (2*size + PAGE_SIZE + (PAGE_SIZE - 1)) & ~(PAGE_SIZE - 1);
