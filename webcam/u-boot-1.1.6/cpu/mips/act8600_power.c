...
static int act8600_write(unsigned char reg, int bytes, void *src)
{
	...
	int i;
#if 0
	serial_puts("act8600: Write register --");
...
static int act8600_read(unsigned char reg, int bytes, void *dest)
{
	...
	if (ret < 0)
		return ret;
#if 0
	serial_puts("act8600: Read register --");
	serial_put_hex((unsigned int)reg);
	serial_puts("data: ");
	for (i = 0; i < bytes; i++) {
		serial_put_hex(buf[i]);
...
int act8600_ldo_enable(int voltage_set_reg, int voltage)
{
	...
	if(voltage_set_reg > ACT8600_LDO3_VOLTAGE_SET){
		do{
  		...
  		if(timeout <= 0){
			serial_puts("act8600: Write timeout !!!!!!!");
			return 1;
		}
	}
	else{
...
void act8600_vbus_enable(void)
{
/*
 * enable Q1 disable Q3
 */
	act8600_reg_write(ACT8600_OTG_CON, ACT8600_OTG_CON_Q1);
}
...
void act8600_ldo_hdmi_poweron(void)
{
	...
	if(error)
		serial_puts("act8600: enable hdmi1.8v erro !!!");
	error = act8600_ldo_enable(ACT8600_LDO_HDMI_3_3,ACT8600_HDMI_3_3);
	if(error)
		serial_puts("act8600: enable hdmi3.3v erro !!!");
}
void act8600_ldo_hdmi_poweroff(void)
{
	...
	if(error)
		serial_puts("act8600: disable hdmi1.8v erro !!!");
	error = act8600_ldo_disable(ACT8600_LDO_HDMI_3_3);
	if(error)
		serial_puts("act8600: disable hdmi3.3v erro !!!");
}
#endif
#endif
