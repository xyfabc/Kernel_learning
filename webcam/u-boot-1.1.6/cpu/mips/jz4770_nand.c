...
static int jzsoc_nand_bch_correct_data(struct mtd_info *mtd, u_char * dat, u_char * read_ecc, u_char * calc_ecc)
{
	...
	if (stat & BCH_INTS_ERR) {
		...
		if (stat & BCH_INTS_UNCOR) {
			...
			printk("REG_BCH_CR=0x%08x, REG_BCH_CNT=0x%08x, stat=0x%08x\n", REG_BCH_CR, REG_BCH_CNT, stat);
			return -1;
		} else {
...
void board_nand_init(struct nand_chip *nand)
{
	...
	if (CFG_NAND_BCH_BIT == 24) {
		...
		par_size = 65;
		nand->eccmode = NAND_ECC_HW33_512;
	} else if (CFG_NAND_BCH_BIT == 16) {
		par_size = 52;
		nand->eccmode = NAND_ECC_HW26_512;
	} else if (CFG_NAND_BCH_BIT == 12) {
		par_size = 39;
		nand->eccmode = NAND_ECC_HW20_512;
	} else if (CFG_NAND_BCH_BIT == 8) {
		par_size = 26;
