...
void apll_init(void)
{
	...
	int apll_out, uhc_cdr;

        /** divisors, 
	 *  for jz4780 ,P:H2:H0:L2:C
	 *  DIV should be one of [1, 2, 3, 4, 5, 6, ..., 14]
         */
	int div[5] = {8, 4, 3, 2, 1};	 // for 1G
	//int div[5] = {12, 6, 4, 2, 1}; // for 1.2G
	//int div[5] = {12, 6, 5, 2, 1}; // for 1.4G
	//int div[5] = {4, 2, 2, 2, 1};

	/* Init APLL */
//	REG_CPM_CPAPCR = CPAPCR_M_N_OD | CPM_CPXPCR_XPLLEN;
//	while (!(REG_CPM_CPAPCR & CPM_CPXPCR_XPLL_ON)) ;
...
//			(CPM_PLL_SEL_SRC << CPM_CPCCR_SEL_H0PLL_BIT) | (CPM_PLL_SEL_SRC << CPM_CPCCR_SEL_H2PLL_BIT);
	 ...
	 */
#if 0
	uhc_cdr = CPM_UHCCDR_UHCS_SRC | CPM_UHCCDR_CE_UHC;
	uhc_cdr |= apll_out / 48000000 * 2 - 1;
	REG_CPM_UHCCDR = uhc_cdr;
	while (REG_CPM_UHCCDR & CPM_UHCCDR_UHC_BUSY) ;
#endif
}
...
{
        /* Init MPLL */
        REG_CPM_CPMPCR = CPMPCR_M_N_OD | CPM_CPXPCR_XPLLEN;
	while (!(REG_CPM_CPMPCR & CPM_CPXPCR_XPLL_ON)) ;
	 ...
	 */
	__cpm_start_ddr0();
	REG_CPM_DDCDR = CPM_DDRCDR_DCS_MPLL | CPM_DDRCDR_CE_DDR | (CFG_DIV - 1);
...
static void calc_clocks(void)
{
	...
	//apll_out = __cpm_get_xpllout(SCLK_APLL);
	apll_out = __cpm_get_xpllout(SCLK_MPLL);
