...
#if defined(DEBUG) && !defined(CONFIG_NAND_SPL)
...
void lpddr2_init(void)
{
        ...
        cmd.bank = 0;
        cmd.addr = 0x0 << 8 | 0x3f; // 8'h00, 8'h3f
        cmd.dtp = 5;    // tMRD
        cmd.cmd = 0x1;  // LOAD_MODE mr63
        to_cmd_format(cmd, word);
        dprintf("LOAD MODE MR63:\n");
        write_dcu_cache(word, 4);
        ...
        cmd.bank = 0;
        cmd.addr = 0xff << 8 | 0xa; // 8'hff, 8'h0a
        cmd.dtp = 5;    // tMRD
        cmd.cmd = 0x1;  // LOAD_MODE mr10
        to_cmd_format(cmd, word);
        dprintf("LOAD MODE MR10:\n");
        write_dcu_cache(word, 4);
        ...
        cmd.bank = 0;
        cmd.addr = 0x43 << 8 | 0x1; // WR = 4, BL = 8
        cmd.dtp = 5;    // tMRD
        cmd.cmd = 0x1;  // LOAD_MODE mr1
        to_cmd_format(cmd, word);
        dprintf("LOAD MODE MR1:\n");
        write_dcu_cache(word, 4);

        cmd.bank = 0;
        cmd.addr = 0x3 << 8 | 0x2; // RL = 5 / WL = 2
        cmd.dtp = 5;    // tMRD
        cmd.cmd = 0x1;  // LOAD_MODE mr2
        to_cmd_format(cmd, word);
        dprintf("LOAD MODE MR2:\n");
        write_dcu_cache(word, 4);

        cmd.bank = 0;
        cmd.addr = 0x2 << 8 | 0x3; // 48-ohm
        cmd.dtp = 5;    // tMRD
        cmd.cmd = 0x1;  // LOAD_MODE mr3
        to_cmd_format(cmd, word);
        dprintf("LOAD MODE MR3:\n");
        write_dcu_cache(word, 4);

        REG_DDRP_DCURR = 0x801;

        while (!(REG_DDRP_DCUSR0 & 0x1))
                ;

        serial_puts("lpddr2 init over!\n");
        dprintf("====================================================================\n");
        dprintf("DCUAR: 0x%08x\n", REG_DDRP_DCUAR);
        dprintf("DCURR: 0x%08x\n", REG_DDRP_DCURR);
        dprintf("DCUSR0: 0x%08x\n", REG_DDRP_DCUSR0);
...
serial_put_hex(data);
serial_puts("\texpect:");
serial_put_hex(expect);
serial_puts("\tmask:");
serial_put_hex(MASK(byte));
#endif
			res = 0;
			break;
		} else {
...
#ifdef SERIAL
serial_puts("ddr_test_read error!!!\n");
#endif
				if (end - start > 0) {
					dprintf("!!!!!!found!\n");
...
serial_put_hex(byte);
			...
			if (middle != 0xffffffff) {
				...
				dprintf("===================================\n");
#ifdef SERIAL
serial_puts("===> passing configuration:DXDQSTR::");
serial_put_hex(REG_DDRP_DXDQSTR(byte));
#endif
			} else {
				dprintf("=====================================================================\n");
...
#ifdef SERIAL
serial_puts("<=== No passing configuration is possbile for this rank at byte:");
serial_put_hex(byte);
#endif
				dprintf("=====================================================================\n");
				res = 1;
