...
static int global_phy_id;
static int count = 0;
static u32 mii_read(int phy_id, int reg_offset);

static void phy_regis_dump(void)
{
	printf("==================================================\n");
	printf("MII_CR = 0x%04x, MII_SR = 0x%04x\n",
			0xffff & mii_read(global_phy_id, MII_CR), 0xffff & mii_read(global_phy_id, MII_SR));
	printf("MII_ID0 = 0x%04x, MII_ID1 = 0x%04x\n",
			0xffff & mii_read(global_phy_id, MII_ID0), 0xffff & mii_read(global_phy_id, MII_ID1));
	printf("MII_ANA = 0x%04x, MII_ANLPA = 0x%04x\n",
			0xffff & mii_read(global_phy_id, MII_ANA), 0xffff & mii_read(global_phy_id, MII_ANLPA));
	printf("MII_ANE = 0x%04x, MII_ANP = 0x%04x\n",
			0xffff & mii_read(global_phy_id, MII_ANE), 0xffff & mii_read(global_phy_id, MII_ANP));
	printf("==================================================\n");
}
...
#if 1
	for (i = 0; i < MAX_WAIT && __mac_mii_is_busy(); i++, udelay(1)) {
		//		printf("*");
	}
...
static u32 mii_write(int phy_id, int reg_offset, u16 value)
{
	u32     i;

	__mac_send_mii_write_cmd(phy_id, reg_offset, value);
...
static int jz_send(struct eth_device* dev, volatile void *packet, int length)
{
	...
	jz_flush_dcache();
#if 0
	unsigned char *pdata = (unsigned char *)desc->pkt_start_addr;
	int p;
	
	printf("\nSend %04x: ", 0);
	for (p = 0; p < desc->pkt_size; p++) {
		printf("%02x ", pdata[p]); 
...
#if 0
			unsigned char *pdata = NetRxPackets[next_rx];
			int p;
			printf("\n%04x: ", 0);
			for (p = 0; p < length; p++) {
				printf("%02x ", pdata[p]); 
...
static int jz_init(struct eth_device* dev, bd_t * bd)
{
	...
	__gpio_as_output0(32 * 5 + 22);
	udelay(100000);
	__gpio_as_output1(32 * 5 + 22);
